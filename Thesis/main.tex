...
As the name suggests, Synthetic data is artificially created rather than generated by actual events. This data can be used in binary classification problems where the distribution between classes is imbalanced. The classifier performance can be increased by oversampling the minority class before training a model. Currently, SMOTE-based oversamplers are the most used algorithms, relying on a k-nearest neighbours principle to synthesize new data samples, which can lead to overfitting. This thesis introduces a novel method for data generation called Synthsonic, a probabilistic model which for tabular data generation. Synthsonic uses invertible mathematical transformations, discriminative learning and a Bayesian model to capture all dependencies in a dataset, and uses this to generate new independent data samples.
This novel approach was used in the classification of 27 highly imbalanced datasets and compared to other oversampling techniques. This thesis shows that oversampling provides a small increase in classifier performance scores, but the effect is often minor. On average, Synthsonic performed well on smaller datasets with fewer features and samples.
}
